...
def answer_one():
    ...
    arr = np.linspace(0,10,100).reshape(-1,1)

    for count,i in enumerate([1,3,6,9]):
        poly = PolynomialFeatures(degree=i)
        X_train_poly = poly.fit_transform(X_train.reshape(-1,1))
        X_test_poly = poly.transform(arr)
        linreg = LinearRegression().fit(X_train_poly, y_train)
        ans = linreg.predict(X_test_poly)
        result[count,:] = ans
...
def answer_two():
    ...
    for i,deg in enumerate(range(0,10)):
        ...
        a[i] = r2_score(y_train,linreg.predict(X_poly_train))
        b[i] = r2_score(y_test,linreg.predict(X_poly_test))
...
def answer_three():
    ...
    for i in range(len(c[0])):
        plt.plot(ind[i],c[0][i],'co')
        plt.plot(ind[i],c[1][i],'yv')
    ax = plt.gca()    
...
def answer_four():
    ...
    lasso =  Lasso(alpha=0.01, max_iter = 10000).fit(X_poly_train, y_train)
...
get_ipython().magic('pinfo validation_curve')
...
def answer_six():
    ...
    train_scores,test_scores = validation_curve(SVC(random_state=0),X_subset,y_subset,param_name='gamma',param_range=np.logspace(-4,1,6), cv=3, scoring='accuracy')
